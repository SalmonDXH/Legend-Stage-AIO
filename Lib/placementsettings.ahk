...
skills := [0, 1, 2, "VSJW Summon+Nuke", "Regular Nuke"]
...
global Slot1SkillClick := PlacementGUI.Add("DropDownList", "x60 y315 w160", skills)
...
global Slot2SkillClick := PlacementGUI.Add("DropDownList", "x300 y315 w160", skills)
...
global Slot3SkillClick := PlacementGUI.Add("DropDownList", "x540 y315 w160", skills)
...
global Slot4SkillClick := PlacementGUI.Add("DropDownList", "x780 y315 w160", skills)
Slot4SkillClick.Choose(1)

; Slots
PlacementGUI.Add("Text", "x40 y10 cWhite", "How many slots you have? ")
global SlotPlacement := PlacementGUI.Add("DropDownList", "x210 y10 w50", [6, 5, 4, 3])
PlacementGUI.Add("Text", "x40 y30 cWhite", "Taka and Speed must always be at 2 last slot.`nFor example:`nIf you have 4 slots, Taka and Speed must be slot 3 and 4.")
SlotPlacement.Choose(1)
...
SavePlacement() {
    ...
    slots_path := A_ScriptDir "\Lib\UnitPlacements\slots.txt"
    ...
    text2 := SlotPlacement.value
    try {
        FileDelete(slots_path)
        FileAppend(text2, slots_path, "UTF-8")

    } catch {
        MsgBox("Error writing to file: " slots_path)
    }
...
LoadSlots() {
    file_path := A_ScriptDir "\Lib\UnitPlacements\slots.txt"
    nums := 0
...
PlacementSettingUI() {
    ...
    slotsplacement := LoadSlots()
    SlotPlacement.Choose(slotsplacement)
    if data.Has("Slot1") {
